...
9. **Objetivo prioritario cediendo** — el objetivo más prioritario (el de `prioridad` MENOR: 1 = lo primero) va atrasado mientras uno menos prioritario va al día.
   ...
   **9a — BARATA (PASO 5.5, cada sesión).** Es lookup puro, sin cola de viabilidad. CUENTA: sea `P` el objetivo activo de `prioridad` menor. Si su `_nota_viabilidad` (o cualquier campo de `objetivo.json`) declara un DÉFICIT vigente, y **el curso que ganó el selector NO aparece en el `cursos_requeridos` de `P`**, entonces se cumple. Es decir: estás a punto de gastar horas en un objetivo secundario mientras el prioritario ya declaró que no le alcanzan.
