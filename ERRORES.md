...
- *2026-07-22* — La alerta **9a** (cableada el 18-jul para el arranque en frío) volvió a usar `cursos_requeridos` como si fuera todo el universo de "sirve a `P`". Con obj-2 activo y `01-aritmetica` ganando el selector por rama F, `01-aritmetica` no aparece en el `cursos_requeridos` de obj-2 (solo entra por `base_por_cierre`/cerradura) — la alerta, leída literal, habría dicho que la clase "sirve solo a [objetivo secundario]" cuando en realidad sostiene a obj-2 directamente y no existe tal secundario. Detectado ANTES de aplicarla (el agente reconoció la ambigüedad y no emitió el aviso falso), no después. → Arreglado el mismo día: la condición ahora exige que el curso no esté ni en `cursos_requeridos` **ni** en `aprender(P) ∪ mantener(P)` antes de disparar. Ver `_protocolos/utilidades.md`, alerta 9a.
