...
- **Incrementa `veces_repasado` en 1** — SIEMPRE que el tema se ejercitó como repaso, acierte o falle. Es el contador acumulativo que, a diferencia de `nivel_repaso`, no se resetea; su cruce con `nivel_repaso` es lo que delata un tema que se resiste (ver la doc del campo y la alerta en `utilidades.md`).
...
- Si falló: baja `nivel_repaso` a 1 y pon `fecha_proximo_repaso` = mañana. Además, registra la dificultad en `dificultades.json` como cualquier otra. (`veces_repasado` ya subió arriba: un fallo cuenta como repaso hecho.)
...
- `veces_repasado` — contador ACUMULATIVO de repasos hechos sobre el tema (no se resetea al fallar, a diferencia de `nivel_repaso`). Diagnóstico: muchos repasos + `nivel_repaso` bajo = tema que se resiste y cuya profundidad hay que re-verificar. Lo incrementa el "Protocolo al CERRAR una clase". Arranca en 0
